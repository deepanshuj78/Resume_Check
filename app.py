...
st.markdown(
    ...
    <style>
        ...
        {
            background-color: #70c0d7 !important;
        ...
        .dev-header {
            ...
            [data-testid="stSidebar"] div[data-testid="stCheckbox"] button[aria-checked="true"] 
            {
        ...
        /* Click contraction feedback physics */
        [data-testid="stSidebar"] button[kind="secondary"]:active {
            transform: scale(0.97) !important;
        }

        /* Softened, uniformly balanced divider line */
...
with st.sidebar:
    ...
    st.session_state["ats_run_completed"] = st.toggle(
...
if st.session_state.get("ats_run_completed", False):
    if not st.session_state.get("quiz_generation_triggered", False):
...
if st.session_state.get("quiz_generation_triggered", False) and st.session_state.get("generated_quiz_data") is None:
    quiz_questions = []
    topics_found = list(st.session_state.get("missing_skills_set", []))
    if not topics_found:
    ...
    for topic in topics_found:
        ...
        if not topic_questions:
            continue
        available_questions = [q for q in topic_questions if q.get("id") not in st.session_state.get("used_question_ids", set())]
...
if st.session_state.get("generated_quiz_data"):
    ...
    with st.container(border=True):
        st.markdown(f"### 📋 Technical Screening Quiz")
        st.caption(f"Targeting parsed resume skill gaps: `{quiz['keywords']}`")
        st.divider()
        for idx, item in enumerate(quiz["questions"], start=1):
            ...
            selected_option = st.radio(
                ...
                index=None,
                key=f"mcq_{item['topic']}_{item.get('id', idx)}",
                label_visibility="collapsed"
            )
